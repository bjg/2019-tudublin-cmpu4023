...
let postHmacProducts = (jwtToken, accesskey, signature, serversecret) => {
    return new Promise((resolve, reject) => {
        axios.post(DOMAIN + "/products_hmac", BODY,{
            "headers": {
...
login(username, password, client.getPrime().toString('base64'), client.getGenerator().toString('base64'), clientKey.toString('base64'))
    .then(data => {
        ...
        const clientSignature = crypto.createHmac("md5", secret)
                    .update(BODYSTRINGIFIED + accesskey)
                    .digest("base64");

        postHmacProducts(data.token, accesskey, clientSignature, data.server_secret)
